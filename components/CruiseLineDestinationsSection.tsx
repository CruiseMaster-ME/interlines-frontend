...
import { buildOdysseusDestinationUrl } from "@/lib/odysseus";

type CruiseLineDestinationsSectionProps = {
  items: readonly string[];
};

export default function CruiseLineDestinationsSection({
  items,
}: CruiseLineDestinationsSectionProps) {
      ...
      {items.length > 0 ? (
        <div className="mt-5 flex flex-wrap items-center gap-y-2 text-[14px] leading-8 text-[var(--interlines-slate)] sm:text-[15px]">
          {items.map((item, index) => {
            const href = isAuthenticated ? buildOdysseusDestinationUrl(item) : null;
